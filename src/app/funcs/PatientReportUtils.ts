...
export async function downloadPatientReport() {
  try {
    ...
    const accentColor: [number, number, number] = [37, 99, 235]; // Red
    ...
    doc.setFontSize(14);
    doc.text(
      `${auth.userDetails?.name || "Patient"} - Medical Summary`,
      105,
      20,
      {
        align: "center",
    ...
    doc.text(
      `Country: ${auth.userDetails?.country || "N/A"}`,
      15,
      currentY + 18
    );
      ...
      doctors.forEach((doctor, index) => {
        ...
        if (doctorSchedule && doctorSchedule.schedules.length > 0) {
          ...
          doctorSchedule.schedules.forEach((schedule, sIndex) => {
            ...
            doc.text(
              `Date: ${
                schedule.schedule ? formatDate(schedule.schedule) : "N/A"
              }`,
              25,
              currentY
            );
